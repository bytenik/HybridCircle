...
/* end of major run() macros */
	...
	case OP_LE:
		...
		} else if (lhs.type == TYPE_OBJ && rhs.type != TYPE_OBJ) { // HybridCircle 2.1h
			if (!valid(lhs.v.obj))
				PUSH_ERROR(E_INVIND);
			else {
				Var arglist;
	...
	case OP_GET_PROP:
		...
		if (obj.type == TYPE_WAIF && propname.type == TYPE_STR) {
			...
			PUSH_ERROR(err);
		} else if (propname.type == TYPE_STR && obj.type == TYPE_LIST) {
			Var ans = new_list(0);
			enum error oerr;
			
			enum error
			listinlist(Var passobj, Var *outerlist)
			{
				long cnt;
				Var innerlist = new_list(0);

				for (cnt = 1; cnt <= passobj.v.list[0].v.num; cnt++)
				{
					if(passobj.v.list[cnt].type == TYPE_LIST) {
						enum error err;
						err = listinlist(passobj.v.list[cnt], &innerlist);
						if (err != E_NONE)
							return err;
					} else if((passobj.v.list[cnt].type != TYPE_OBJ
								&& passobj.v.list[cnt].type != TYPE_WAIF)
								|| propname.type != TYPE_STR)	{
						free_var(propname);
						free_var(obj);
						return E_TYPE;
					} else if (!valid(passobj.v.list[cnt].v.obj)) {
						free_var(propname);
						free_var(obj);
						return E_INVIND;
					} else if (passobj.v.list[cnt].type == TYPE_WAIF) {
						enum error err;
						err = waif_get_prop(obj.v.waif, propname.v.str, &prop, RUN_ACTIV.progr);
						if(err != E_NONE)
							return err;
						innerlist = listappend(innerlist, var_dup(prop));
					} else {
						db_prop_handle h;

						h = db_find_property(passobj.v.list[cnt].v.obj, propname.v.str, &prop);
						if(!h.ptr) {
							free_var(propname);
							free_var(obj);
							return E_PROPNF;
							...
							innerlist = listappend(innerlist, prop);
						else
							innerlist = listappend(innerlist, var_dup(prop));
					}
				}

				*outerlist = listappend(*outerlist, innerlist);
				return E_NONE;
			}

			oerr = listinlist(obj, &ans);
			if (oerr != E_NONE)
				PUSH_ERROR(oerr);
			PUSH(ans.v.list[1]);
	...
	case OP_PUT_PROP:
		...
		if (obj.type == TYPE_WAIF && propname.type == TYPE_STR) {
		    ...
		    free_var(obj);
		    PUSH_ERROR(E_TYPE);
		} else if (!valid(obj.v.obj)/*!valid(obj.v.obj)*/) {
		    free_var(rhs);
		    free_var(propname);
		    free_var(obj);
		    PUSH_ERROR(E_INVIND);
			...
			case BP_NAME:
				...
				err = E_TYPE;
			    else if (is_user(obj.v.obj) && !db_can_power_hack(obj.v.obj, progr))
				err = E_PERM;
				else if (!db_can_hack(obj.v.obj, progr))
				err = E_PERM;
			    break;
			case BP_OWNER:
			    if (rhs.type != TYPE_OBJ)
				err = E_TYPE;
			    if (!db_can_power_hack(obj.v.obj, progr))
				err = E_PERM;
			    break;
			case BP_PROGRAMMER:
			case BP_WIZARD:
			...
			case BP_H:
			case BP_F:
			    if(!db_can_hack(obj.v.obj, progr))
				err = E_PERM;
			    break;
			case BP_P:
				if(!db_can_power_hack(obj.v.obj, progr))
			...
			//char *verbname;
			//char verbname[2000];
			//enum error err2 = E_NONE;
			//Var arguments;
			//Var returndata;
			//verbname = malloc(strlen(propname.v.str)+10);
			//verbname = mymalloc(1000,M_STRING);
			
		//	strcpy(verbname, "_prop_set_");
		//	strcat(verbname,propname.v.str);
		//	arguments=new_list(1);
		//	arguments.v.list[1].type = rhs.type;
		//	arguments.v.list[1] = rhs;
			//oklog("%s\n",verbname);
		//	err2 = call_verb(obj.v.obj,verbname,arguments,0);
			//err2 = E_VERBNF;
			
		//	if(err2 == E_NONE)
		//	{
		//		returndata = POP();
		//		db_set_property_value(h, var_ref(returndata));
		//		//free_var(rhs);
		//		PUSH(returndata);
		//	}
		//	else
...
/* Scattered FOR (version 2.1h) */
 		case EOP_FOR_SCATTER:
 ...
 
 			{
 			    ...
 			    list =
 				NEXT_TOP_RT_VALUE.v.list[TOP_RT_VALUE.v.
 							 num++];
