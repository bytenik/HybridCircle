...
get_bi_value(db_prop_handle h, Var * value)
{
	...
	case BP_ADMIN:
	value->type = TYPE_INT;
	value->v.num = db_object_has_flag(oid, FLAG_ADMIN);
	break;
    case BP_R:
	value->type = TYPE_INT;
	...
	case BP_P:
	value->type = TYPE_INT;
	value->v.num = db_object_has_flag(oid, FLAG_PROTECTED);
	break;
	case BP_M:
	value->type = TYPE_INT;
	value->v.num = db_object_has_flag(oid, FLAG_MONITOR);
	break;
	case BP_H:
	value->type = TYPE_INT;
	value->v.num = db_object_has_flag(oid, FLAG_HIDDEN);
	break;
    case BP_LOCATION:
	value->type = TYPE_OBJ;
...
int
db_property_has_flag(db_prop_handle h, db_prop_flag flag)
{
...
{
    Objid owner;
	return (db_property_has_flag(h, flag)
	    || progr == (owner=db_property_owner(h))
	    || (is_wizard(progr) && !db_property_has_flag(h, PF_PROTECTED))
...
char rcsid_db_properties[] = "$Id: db_properties.c,v 1.1 2002/02/22 19:17:09 bytenik Exp $";

/* 
 * $Log: db_properties.c,v $
 * Revision 1.1  2002/02/22 19:17:09  bytenik
 * Initial revision
 *
