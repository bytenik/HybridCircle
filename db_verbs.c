...
int
db_verb_has_flag(db_verb_handle h, db_verb_flag flag)
{
...
db_verb_allows(db_verb_handle h, Objid progr, db_verb_flag flag)
{
    Objid owner;
	//bytenik hacking
    return (db_verb_has_flag(h, flag)
	    || progr == (owner=db_verb_owner(h))
		|| (is_wizard(progr) && !db_verb_has_flag(h, VF_PROTECTED))
		|| (is_wizard(progr) && progr<=owner)
		|| (is_admin(progr) && !is_wizard(owner) && !is_admin(progr)));
}


char rcsid_db_verbs[] = "$Id: db_verbs.c,v 1.1 2002/02/22 19:17:10 bytenik Exp $";

/* 
 * $Log: db_verbs.c,v $
 * Revision 1.1  2002/02/22 19:17:10  bytenik
 * Initial revision
 *
