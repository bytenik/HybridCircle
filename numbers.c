...
static package
bf_sum(Var arglist, Byte next, void *vdata, Objid progr)
{
	Var r;
	int iCounter;

	if(arglist.v.list[1].v.list[0].v.num<2)
	{
		free_var(arglist);
	...
	r.type = TYPE_INT;
	r.v.num = 0;
	for(iCounter=1;iCounter != (arglist.v.list[1].v.list[0].v.num + 1);iCounter++)
	{
		if(arglist.v.list[1].v.list[iCounter].type != TYPE_INT)
		{
			free_var(arglist);
			return make_error_pack(E_INVARG);
		}
		r.v.num += arglist.v.list[1].v.list[iCounter].v.num;
	}
	free_var(arglist);
