...
///// BEGIN LUKE-JR ADDED BLOCK /////
    if (desc.type == TYPE_LIST &&
        desc.v.list[0].v.num == 2 &&
     desc.v.list[1].type == TYPE_STR && desc.v.list[2].type == TYPE_INT) {
        // {IP, Port}
        bind_addr = lookup_addr_from_name(desc.v.list[1].v.str, timeout);
...
/////  END LUKE-JR ADDED BLOCK  /////
    ...
    if (desc.type == TYPE_LIST) {
        // *canon = new_list(2);
        *canon = var_ref(desc);
        //canon->v.list[1].type = TYPE_STR;
        //canon->v.list[1].v.str = desc.v.list[1].str;
        canon->v.list[2].type = TYPE_INT;
        canon->v.list[2].v.num = ntohs(address.sin_port);
    } else {
/////  END LUKE-JR ADDED BLOCK  /////
    canon->type = TYPE_INT;
...
///// BEGIN LUKE-JR ADDED BLOCK /////
    if (canon->type == TYPE_LIST) {
        stream_printf(st, "IP %d ", desc.v.list[1].v.str);
        stream_printf(st, "port %d", canon->v.list[2].v.num);
    } else {
/////  END LUKE-JR ADDED BLOCK  /////
...
char rcsid_net_sysv_tcp[] = "$Id: net_sysv_tcp.c,v 1.1 2002/02/22 19:17:45 bytenik Exp $";

/* $Log: net_sysv_tcp.c,v $
/* Revision 1.1  2002/02/22 19:17:45  bytenik
/* Initial revision
/*
/* Revision 1.3  2001/01/29 21:39:25  luke-jr
/* Fixed a bug caused by the RECYCLED BLOCK stuff...
/*
/* Revision 1.2  2001/01/28 17:21:33  luke-jr
