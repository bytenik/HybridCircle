...
{
    unsigned int i; //int to unsigned int
...
{
    unsigned int i; //int to unsigned
    Loop *loop;
...
generate_stmt(Stmt * stmt, State * state)
{
	...
	/* Scattered FOR (version 2.1h) */
 	case STMT_SCATFOR:
 		...
 		end_label = add_label(state);
 		enter_loop(stmt->s.scatfor.scat->id, loop_top,
...
char rcsid_code_gen[] = "$Id: code_gen.c,v 1.2 2002/04/09 01:35:49 luke-jr Exp $";
 ...
 *
 * Revision 1.1.1.1  2002/02/22 19:16:56  bytenik
