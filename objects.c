...
/*
	if ((valid(parent) ? !db_object_allows(parent, progr, FLAG_FERTILE)
	     : (parent != NOTHING))
	    || ((owner != progr && !is_wizard(progr)) || (is_wizard(progr) && db_object_has_flag(parent,FLAG_PROTECTED) && db_object_owner(parent)!=progr && db_object_owner(parent)>progr)))
	    return make_error_pack(E_PERM);
*/
/*	if ( (  valid(parent) ?
    ...
    || ( db_object_has_flag(parent, FLAG_PROTECTED)
          && db_object_owner(parent) < progr )
	|| ( db_object_has_flag(progr, FLAG_ADMIN)
		  && !db_object_has_flag(owner, FLAG_WIZARD)
		  && !db_object_has_flag(owner, FLAG_ADMIN) )
   )    return make_error_pack(E_PERM);
   */
	if ( (  valid(parent) ?
       !db_object_allows(parent, progr, FLAG_FERTILE)
       : (parent != NOTHING)  ))
		return make_error_pack(E_PERM);
	else if (valid(owner) && !decr_quota(owner))
...
static package
bf_create_young(Var arglist, Byte next, void *vdata, Objid progr)
{				/* (parent [, owner]) */
    Objid *data = vdata;
    Var r;
...
register_objects(void)
{
    ...
    register_function_with_read_write("create", 1, 2, bf_create,
				      ...
				      TYPE_OBJ, TYPE_OBJ);
    register_function_with_read_write("create_young", 1, 2, bf_create_young,
				      bf_create_read, bf_create_write,
				      TYPE_OBJ, TYPE_OBJ);
...
char rcsid_objects[] = "$Id: objects.c,v 1.1 2002/02/22 19:17:48 bytenik Exp $";
