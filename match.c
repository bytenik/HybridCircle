...
match_object(Objid player, const char *name)
{
    if (name[0] == '\0')
	return NOTHING;
    if (name[0] == '#' && is_numeric((char *)name+1)) {
	char *p;
	...
	if (name[0] == '#')
	{
		...
		enabled = get_server_option(SYSTEM_OBJECT, "named_field", &value);

		if(value.type == TYPE_OBJ && valid(value.v.obj))
			return match_contents(value.v.obj, name+1);
		else if(value.type == TYPE_INT && value.v.num)
			return match_contents(SYSTEM_OBJECT, name+1);
	}
...
char rcsid_match[] = "$Id: match.c,v 1.1 2002/02/22 19:17:34 bytenik Exp $";

/* 
 * $Log: match.c,v $
 * Revision 1.1  2002/02/22 19:17:34  bytenik
 * Initial revision
 *
