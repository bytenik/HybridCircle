...
extern Objid db_create_object(void);
				 ...
				 */
extern Objid db_set_object_number(Objid, Objid);
				/* Sets an object's number
				*/
...
typedef enum {
    ...
    FLAG_READ, FLAG_WRITE, FLAG_OBSOLETE_2, FLAG_FERTILE, FLAG_PROTECTED, FLAG_ADMIN, FLAG_MONITOR, FLAG_HIDDEN
//    FLAG_READ, FLAG_WRITE, FLAG_PROTECTED, FLAG_FERTILE
} db_object_flag;
...
extern int is_admin(Objid oid);
extern int is_programmer(Objid oid);
extern int is_user(Objid oid);
extern int db_can_hack(Objid oid, Objid progr);
extern int db_can_power_hack(Objid oid, Objid progr);
 ...
 *
 * Revision 1.2  2001/01/28 16:49:48  bytenik
 * Added externs for db_set_object_number
 *
