...
/*****************************************************************************
   This file contains the File Utilities Package, version 1.8, originally
	 developed by Jaime Prilusky and Gustavo Glusman (see above), modified to
	 work under Win32 as a "plug-in" module for WinMOO.

	 My copyright notice appears below; you are free to use and redistribute
	 this and accomanying files subject to the terms below and in FUP.copyright,
	 the copyright terms for the original work.
 *****************************************************************************/

/**********************************************************************
 Copyright (c) 1997 Christopher Unkel.  All rights reserved.
 
 Use and copying of the software and preparation and distribution of
 derivative works is permitted provided this copyright notice is
 left intact.  Any distribution of this software or derivative works
 must comply with all applicable United States export control laws.
 
 This software is provided AS IS, without express or implied warranty
 of any kind.  Christopher Unkel makes no representations concerning
 either the merchantability of this software or the suitability of
 this software of any particular purpose.
 **********************************************************************/
...
/* filechmod would be essentially meaningless under Win32;
   filerun is not ported */

/* #define INCLUDE_FILERUN */
/* #define INCLUDE_FILECHMOD */

/* Unfortunately, the paths are relative to the server executable.
   This highlights the need for some sort of configuration mechanism.
*/
...
build_dir_name(const char *thePathStr, char *theDirName, char spec)
{
    ...
    switch (spec)
	    ...
	    case 'd':
	      ...
	      break;
	    case 'r':
	      if ((_access (theDirName, 0x02)) !=0) return E_PERM;
	      break;
	    case 'w':
	      if ((_access (theDirName, 0x04)) !=0) return E_PERM;
	      break;
	    case 'x':
	      if ((_access (theDirName, 0x00)) !=0) return E_PERM;
	      break;
	    default:
	        return E_ARGS;
	    }
...
char spec;
{
    ...
    switch (spec)
	    ...
	    case 'd':
	      ...
	      break;
	    case 'r':
	      if ((_access (theFileName, 0x02)) !=0) return E_PERM;
	      break;
	    case 'w':
	      if ((_access (theFileName, 0x04)) !=0) return E_PERM;
	      break;
		  /* x access isn't meaningful... */
	    case 'x':
	      if ((_access (theFileName, 0x00)) !=0) return E_PERM;
	      break;
	    default:
	        return E_ARGS;
	    }
...
/* bf_filelist is the most heavily modified function for Win32 plugin support; the
   Microsoft C runtime libraries, at least, don't seem to support standard dirent
	 stuff, so all calls are replaced with appropriate Win32 API calls to
...
{ /* (directory) */
    ...
    if (result != E_NONE) {
		...
		if ((findhandle=FindFirstFile(rootDir, &finddata))==INVALID_HANDLE_VALUE) {
				free_var(arglist);
				return make_error_pack(E_INVARG);
		}

		rootDir[strlen(rootDir)-2]='\0';
   ...
   if (arglist.v.list[0].v.num > 1) {
    ...
    theline.type = TYPE_STR;
		...
		while (keepgoing) {
				if (strncmp(finddata.cFileName, ".", 1)) {
						sprintf(dirName,"%s/%s", rootDir, finddata.cFileName);
						if (finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
								theline.v.str = str_dup(finddata.cFileName);
								listOfDirs = listappend(listOfDirs, theline);
						} else {
						if ((srchlen==0) ||
								(matches(finddata.cFileName,arglist.v.list[2].v.str))) {
								theline.v.str = str_dup(finddata.cFileName);
								listOfFiles = listappend(listOfFiles, theline);
						}
						}
				}
				keepgoing = FindNextFile(findhandle, &finddata);
		}
...
/* plugin_version: return a string representing the current version of this
   plugin. */
...
/* plugin_name: return a string that is the name of the plugin */
const char * plugin_name(void)
{
	return "FUP";
}

/* plugin_banner: called by the server to provide the plug-in an opportunity
   to print a pretty banner message.  Use oklog to do this. */
void plugin_banner(void)
{
  oklog("          (Using File Utilities Package plug-in version %s)\n", FUP_version);
}

void register_files(void);

/* plugin_register: called by the server at built-in function registration
   time to allow the plug-in to register functions with register_function().
*/
