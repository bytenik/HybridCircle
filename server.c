...
#include <pwd.h> // For setuid() built-in
...
emergency_mode()
{
    ...
    printf("\nLambdaMOO Emergency Wizard Mode\n");
    printf("-------------------------------------------\n");
...
main(int argc, char **argv)
{
	...
	fprintf(stderr, "Usage: %s [-e] [-l log-file] [-i IPonly] %s %s\n",
    ...
    if (IPonly) {
        ...
        desc.v.list[1].v.str = IPonly;
        desc.v.list[2] = rdesc;
    }

    oklog("STARTING: Version %s of the HybridCircle server\n", server_version);
    oklog(" -- Written by ByteNik (David Pfeffer)\n");
    oklog(" -- For help, email DavidP@ByteNik.com or visit HybridMOO at\n");
    oklog("    hybrid-moo.net port 8888.\n");
    oklog(" -- For core updates, visit www.hybrid-moo.net/hybridsphere\n");
    oklog("          (Using %s protocol)\n", network_protocol_name());
    oklog("          (Using File Utilities Package version %s)\n", FUP_version);
    oklog("          (Using +o/-o extentions for override support)\n");
    oklog("          (Using WAIF for lightweight support)\n");
...
#ifndef RUNAS_SERVICE
/* normal command-line initiated version */
int
main(int argc, char **argv)
{
    char       	       *this_program = str_dup(argv[0]);
    ...
    /*  WIN32 -- don't set log file to stderr to avoid doubling
    else
	set_log_file(stderr);
    */
    ...
    if (!db_initialize(&argc, &argv)
	||  !network_initialize(argc, argv, &desc)) {
 	fprintf(stderr, "Usage: %s [-e] [-l log-file] [-p plugin1.dll [-p plugin2.dll] ...] [-o] %s %s\n",
		this_program, db_usage_string(), network_usage_string());
	exit(1);
    }

    if (!execute_timer_initialize()) {
      fprintf(stderr, "Couldn't start execution unit timer; aborting.\n");
      exit(1);
    }
...
/**** built in functions ****/
static package
bf_setuid(Var arglist, Byte next, void *vdata, Objid progr)
{ 
	/* (userID) */
	struct passwd *p = 0;
	const char * uid = arglist.v.list[1].v.str;
	uid_t u;

	if (!is_wizard(progr)) {
		free_var(arglist);
		return make_error_pack(E_PERM);
	} else if (!(p = getpwnam(uid))) {
		free_var(arglist);
...
static package
bf_getuid(Var arglist, Byte next, void *vdata, Objid progr)
{
	/* () */
	Var r;
	struct passwd *p = 0;
	...
	r.type = TYPE_STR;
	r.v.str = str_dup(p->pw_name);

	return make_var_pack(r);
}

static package
bf_crash(Var arglist, Byte next, void *vdata, Objid progr)
{
    if (!is_wizard(progr)) {
	free_var(arglist);
	return make_error_pack(E_PERM);
...
static package
bf_server_name(Var arglist, Byte next, void *vdata, Objid progr)
{
    Var r;
    r.type = TYPE_STR;
    r.v.str = str_dup("HybridCircle");
    free_var(arglist);
    return make_var_pack(r);
}
...
///// BEGIN LUKE-JR ADD BLOCK /////
static package
bf_version_info(Var arglist, Byte next, void *vdata, Objid progr)
{
    Var r;
    r = new_list(3);
    r.v.list[1] = new_list(2);
    r.v.list[1].v.list[1].type = TYPE_STR;
    r.v.list[1].v.list[1].v.str = str_dup("HybridCircle");
    r.v.list[1].v.list[2].type = TYPE_STR;
    r.v.list[1].v.list[2].v.str = str_dup(server_version);
    r.v.list[2] = new_list(2);
    r.v.list[2].v.list[1].type = TYPE_STR;
    r.v.list[2].v.list[1].v.str = str_dup("LambdaMOO");
    r.v.list[2].v.list[2].type = TYPE_STR;
    r.v.list[2].v.list[2].v.str = str_dup(lambda_server_version);
    r.v.list[3] = new_list(2);
    r.v.list[3].v.list[1].type = TYPE_STR;
    r.v.list[3].v.list[1].v.str = str_dup("File Utilities Package");
    r.v.list[3].v.list[2].type = TYPE_STR;
    r.v.list[3].v.list[2].v.str = str_dup(FUP_version);
    return make_var_pack(r);
}
/////  END LUKE-JR ADD BLOCK  /////
...
///// BEGIN LUKE-JR ADD BLOCK /////
static package
bf_set_object_number(Var arglist, Byte next, void *vdata, Objid progr)
{
    Var r;
    Objid o = arglist.v.list[1].v.obj;
    Objid n = arglist.v.list[2].v.obj;
	Objid counter = 0;
    free_var(arglist);
	...
	if(is_wizard(o) && n<o)
	{
		...
		while(counter!=o)
		{
			...
			if(!valid(counter))
				continue;
			if(is_wizard(counter) && counter<progr)
				return make_error_pack(E_PERM);
		}
	}

    r.type = TYPE_OBJ;
    r.v.obj = db_set_object_number(o, n);
    return make_var_pack(r);
}
/////  END LUKE-JR ADD BLOCK  /////
...
//#ifdef OUTBOUND_NETWORK
	if(!server_flag_option("disallow_outbound_network"))
	{
    Var r;
    enum error e;
...
///// BEGIN LUKE-JR ADD BLOCK /////
    register_function("set_object_number", 2, 2, bf_set_object_number,
     TYPE_OBJ, TYPE_OBJ);
    register_function("server_name", 0, 0, bf_server_name);
    register_function("version_info", 0, 0, bf_version_info);
/////  END LUKE-JR ADD BLOCK  /////
	...
	register_function("setuid", 1, 1, bf_setuid, TYPE_STR);
 ...
 * Another bug
 *
 * Revision 1.12  2001/01/29 21:43:18  luke-jr
 * Fixed a bug with the bootup not doing \n
 *
 * Revision 1.11  2001/01/29 21:39:25  luke-jr
 * Fixed a bug caused by the RECYCLED BLOCK stuff...
 *
 * Revision 1.10  2001/01/29 00:01:17  luke-jr
 * Fixed up a few BIs...
 *
 * Revision 1.9  2001/01/28 23:48:10  bytenik
 * Repaired FUP and Builtin Utilities (Martian Utilities)
 *
 * Revision 1.7  2001/01/28 21:01:41  luke-jr
 * Added nice little BI funcs...
 *
 * Revision 1.6  2001/01/28 18:31:58  luke-jr
 * Fixed bf_crash!
 *
 * Revision 1.5  2001/01/28 18:05:39  luke-jr
 * bf_set_object_number added in... Some bugs fixed with bf_crash...
 *
 * Revision 1.4  2001/01/28 17:39:37  bytenik
 * crash() is now a wizard only builtin
 *  -- it SIGSTOPs now, not SIGHALT
 *
<<<<<<< server.c
=======
 * Revision 1.3  2001/01/28 17:21:33  luke-jr
 * IPonly Patch. Compile-Tested. SHOULD work in runtime...
 *
 * Revision 1.2  2001/01/28 17:12:01  bytenik
 * Added crash() built-in
 *
>>>>>>> 1.3
