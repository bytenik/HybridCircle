...
Objid
db_set_object_number(Objid old, Objid new)
{
    Object *o;
...
{
	return (db_can_hack(oid, progr) || db_object_has_flag(oid, f));
}

int
db_can_hack(Objid oid, Objid progr)
{
	Objid owner;
	//oklog("%n ---- %n",oid,progr);
	return (valid(progr) ? (progr == (owner=db_object_owner(oid))
		|| (is_wizard(progr) && !db_object_has_flag(oid, FLAG_PROTECTED))
		|| (is_wizard(progr) && progr<=owner)
		|| (is_admin(progr) && !is_wizard(owner) && !is_admin(owner)) ) : 0);
}
...
int
db_can_see_object(Objid oid, Objid progr)
{
	//return (valid(progr) ? ( ( valid(oid) && (!db_object_has_flag(oid, BP_H)) )
		//|| (db_can_hack(oid, progr)) ) : 0);

	return (valid(progr) ? valid(oid) : 0);
}
...
is_admin(Objid oid)
{
	return valid(oid) && db_object_has_flag(oid, FLAG_ADMIN);
}
